...
.print_service_details <- function(expected,
                                   platform,
                                   station,
                                   time,
                                   via,
  ...
  if (length(station) > 1 & length(via) > 1) { # Services with multiple locations
    ...
    if (length(unique(via)) == 1) {
      ...
      # # Handle services with multiple destinations and different via
    # }
    aux <- .glue_details(expected = expected,
                         platform = platform,
                         station = station[1],
                         time = time,
                         via = via[1],
    ...
    paste0(aux,
           purrr::map_chr(seq_along(station)[-1],
                          function(i) {
                            .glue_details(expected = NA,
                                          platform = NA,
                                          station = paste("and", station[i]),
                                          time = NA,
                                          via = via[i],
                                          show_colours = show_colours)
                          }),
